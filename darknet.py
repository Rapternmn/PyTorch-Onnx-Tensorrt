...
class Darknet(nn.Module):
    def __init__(self, cfgfile):
        ...
        self.seen = 0
        pwd = os.getcwd()
        self.indices = np.load("{}/indices.npy".format(pwd))   ### Layers to delete inorder to reduce memory consumption
    ...
    def forward(self, x, confidence = 0.3, CUDA = 1):
        detections = []
        ...
        for i in range(len(modules)):       
            ...
            if module_type == "convolutional" or module_type == "upsample" or module_type == "maxpool":
                ...
                if i == 0:
                    outputs[i]= self.module_list[i](x)
                else:
                    outputs[i] = self.module_list[i](outputs[i-1])
                    
                if (i - 1) in self.indices:
                    del outputs[i-1]                    ### Delete Previous layer data to save memory
                ...
                outputs[i] = outputs[i-1] + outputs[i+from_]
        ...
        for i,x in enumerate(x_arr):
            x = predict_transform(x, inp_dim, anchors_arr[i], num_classes)
            if not write:
                detections = x
                write = 1
            else:
